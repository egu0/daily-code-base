...
输入一段 Markdown 或 Obsidian 笔记内容，输出结构化摘要。
...
- structured output
- Zod schema
- 让 agent 稳定返回可程序处理的数据
*/

import { Agent, run } from "@openai/agents";
import { z } from "zod";

const summaryType = z.object({
  title: z.string(),
  summary: z.string(),
...
const agent = new Agent({
  ...
  instructions: `generate a structured output from the input text:
1. if the note contains a markdown heading starting with '#', use it as the title. If there is no heading, generate a short and clear title.
2. Keep the summary to 1-2 sentences.
3. For actionItems, include only clear and executable tasks. If there are no real action items, return a empty array.
4. Tags must be lowercase and concise. Use hyphens for multi-word tags (e.g. 'web-design').
`,
  outputType: summaryType,
  model: "gpt-5.4",
  modelSettings: {
    reasoning: {
      effort: "medium",
...
const source = `
*[Please enjoy this article on its own webpage](https://daringfireball.net/2026/05/what_is_a_dickover). Trust me.*
...
Dickovers are a veritable scourge. They’re so common they’re effectively part of the firmament. I started calling these things [*dickpanels* in 2022](https://daringfireball.net/linked/2022/08/02/banish), but when *dickover* popped into my head last week,[^1] I couldn’t shake the feeling that it’s a better term for these ubiquitous odious irritations. You can hardly go anywhere on the web without getting dicked over by a dickover. They often pester you about permitting cookies, like [this one from Euronews](https://daringfireball.net/misc/2026/05/dickover-euronews.jpeg) or [this one from Gallup](https://daringfireball.net/misc/2026/05/dickover-gallup-cookie.png). This malicious design pattern is so ubiquitous that it has spread even to personal blogs, [like this one from my friend Om Malik](https://daringfireball.net/misc/2026/05/dickover-om-newsletter.png), and to [great brands like Field Notes](https://daringfireball.net/misc/2026/05/dickover-field-notes.jpeg), both asking you to sign up for their newsletters.

The homepage for *every single* blog hosted by Substack shows a particularly pernicious dickover on its homepage. The Substack dickover doesn’t even look like a panel. It’s a full-screen curtain designed and worded to suggest, strongly, that you *need* to sign up for the blog’s email newsletter just to read anything. The dismissal button for the Substack dickover is a small text link — that doesn’t look anything like a button — that says something like “No thanks” (e.g. [Paul Krugman](https://daringfireball.net/misc/2026/05/dickover-substack-krugman.jpeg), [Matt Yglesias](https://daringfireball.net/misc/2026/05/dickover-substack-yglesias.jpeg)) or something that adds insult to injury with a cloyingly saccharine label like “Just gimme that content!” (e.g. [Volts](https://daringfireball.net/misc/2026/05/dickover-substack-volts.jpeg)).

[Here’s one from The Philadelphia Inquirer](https://daringfireball.net/misc/2026/05/dickover-inquirer-shore-texts.jpeg), for which I pay $20/month to subscribe, asking me to sign up for SMS text messages about the Jersey shore, while I’m logged into their cursed website, before they’ll let me see the article I came to read. Every time I see one of these I think about unsubscribing. I’m paying them to abuse my time and attention. I started capturing screenshots of every dickover I saw when I started working on this article, and I soon had to give up because I was collecting too many of them. But [this one from Tom’s Hardware](https://daringfireball.net/misc/2026/05/dickover-dickover-tomshardware.png) I actually enjoyed, because their own dickover got dicked over by one of their own fucking ads in a JavaScript Z-axis slapfight.
...
- [Dickbars](https://daringfireball.net/linked/2011/03/06/dickbar) are related to dickovers, but are far lesser crimes against design and user experience. A dickbar is a non-modal popover that obstructs only a portion of the underlying content, often just a short horizontal strip. If dickovers are design felonies, dickbars are misdemeanors. Here are typical dickbar examples [on desktop](https://daringfireball.net/misc/2026/05/dickbar-ruxton.jpeg) and [mobile](https://daringfireball.net/misc/2026/05/dickbar-storm-type.png) layouts. Here’s a relatively attractive one [from Apple on its Newsroom blog](https://daringfireball.net/misc/2026/05/dickbar-apple-newsroom.png), and one [from the excellent Acquired podcast](https://daringfireball.net/misc/2026/05/dickbar-corner-acquired.jpeg) that is gracious enough to tuck itself into a corner. Here’s an [obnoxious one from the Four Seasons](https://daringfireball.net/misc/2026/05/dickbar-big-fourseasons.jpeg) that is large enough to edge toward dickover consideration. What makes dickbars lesser offenses is that they do not obscure the entirety of the underlying content, and thus do not demand mandatory action to dismiss them. You can view and scroll the page below them. What makes dickbars crimes is that they still obstruct and distract. (Horizontal dickbars, the most common form, interfere with paging via the space bar one screenful at a time. The content scrolls by the height of the webpage, not the height of the webpage minus the height of the dickbar. Thus the dickbar covers unread text each time you page down.)
- All dickovers are modal blockers, but not all modal blockers are dickovers. A paywall sign-up / sign-in panel, for example, is not a dickover. Paywalls are, at times, annoying, but one of the defining aspects of a dickover is that they are unnecessary. Cookie permissions are *un* necessary. Signing up for an email newsletter is *un* necessary. But for paywalled content, asking for sign-up / sign-in is necessary.

---

[^1]: The neologism came to me when I decided to [write about Dropover](https://daringfireball.net/linked/2026/05/15/dropover), a great little drag-and-drop “shelf” [utility for the Mac](https://dropoverapp.com/), shortly after I posted [an item complaining about](https://daringfireball.net/linked/2026/05/14/vestager-ai-safety-institute) a particularly ridiculous cookie modal blocker from Euronews. That Euronews post got me thinking that *dickpanel* just wasn’t sticking. The *dick* part was right but the *panel* part lacked oomph. It didn’t snap. Then I got to writing about Drop *over* and it came to me. That night [I ran a poll on Mastodon asking](https://mastodon.social/@gruber/116575825801893849) “What’s a better word for in-window fake ‘dialog boxes’ that websites (and some apps) use to cover up content?” —  *dickpanel* or *dickover*  — and, with 1,130 responses, *dickover* won by a slim 51–49 percent margin. But after sleeping on it I was convinced *dickover* was the better term, no matter how the poll turned out. Those who voted for *dickpanel* were, judging from their comments, concerned that “over” isn’t descriptive enough. But what makes a neologism stick isn’t descriptiveness or obviousness, but usage. If we use it, it’ll stick. And I, for one, want to use *dickover* because it stings, and it’s fun to sting people who are doing something dickheaded.`;

const source2 = `# Agent SDK 学习
今天看了 structured output 和 tools。
明天要实现一个本地 todo agent。
需要整理 Obsidian 里的相关笔记。`;

const result = await run(agent, source);
const result2 = await run(agent, source2);
