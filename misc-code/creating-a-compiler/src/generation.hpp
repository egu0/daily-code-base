...
public:
    ...
    void gen_stmt(const NodeStmt* stmt)
    {
        struct StmtVisitor {
            ...
            {
                gen.m_output << "    ;; let\n";
                const auto it = std::ranges::find_if(std::as_const(gen.m_vars),
                    [&](const Var& var) {
            ...
            void operator()(const NodeStmtIf* stmt_if) const
            {
                ...
                if (stmt_if->pred.has_value()) {
                    const std::string end_label = gen.create_label() + "_end";
                    gen.m_output << "    jmp " << end_label << "\n";
...
private:
    ...
    */
    ...
    /**
     * 作用域结束时，需要增加 rsp 数值、删除 m_vars 尾部在本作用域添加的变量
    */
    void end_scope()
    {
        std::size_t pop_count = m_vars.size() - m_scopes.back();
        if (pop_count != 0) {
